...
import logging 
from .utils import filter_sentence_too_long, save_weight
from translator.utils.load_weight import load_model
...
def training(language_src: str, 
             ...
             n_decoder_layer: int, 
             dropout: float, 
             save_weight_name: str,
             load_prev_weight: bool = False
             ) -> None: 
    # DATA
    ...
    token_src: Tokenizer = TokenLoad(data=data_source, 
                                     language=language_src).get_token()
    
    token_tgt: Tokenizer = TokenLoad(data=data_source, 
                                     language=language_tgt).get_token()
    ...
    device = setup_device()
    if load_prev_weight:
        model = load_model(save_weight_name)
    else:
    ...
    for epoch in tqdm(range(n_epochs), position =0, desc="Epoch"):
        try:
            ...
            save_weight_bool: str = input(f"Would you like to save the model and the training as '{save_weight_name}' (y/N)")
            if save_weight_bool=="y": 
                save_weight(save_name=save_weight_name, 
                            epoch = epoch, 
                            optimizer=optimizer, 
                            model = model, 
                            language_src = language_src, 
                            ...
                            n_decoder_layer=n_decoder_layer, 
                                            ...
                                            vocab_size_tgt=token_tgt.get_vocab_size(), 
                            dropout= dropout)
                logger.info(f"Weight has been saved as {save_weight_name}")
            else: 
                logger.info("Weights have not been saved.")
            return None
